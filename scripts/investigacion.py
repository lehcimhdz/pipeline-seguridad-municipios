...
def validar_aporte(aporte, config=None):
    ...
    for line in aporte['lineas']:
        ...
        if not all(reference['url'] in line['analisis'] for reference in refs):
            raise ValueError('El análisis debe citar las URL de sus referencias para localizar la evidencia.')
...
def integrar(result):
    ...
    refs = [r for line in statuses for r in line['referencias']]
    if refs:
        values['bibliografia'] += '\n\n' + '\n\n'.join(
...
def narrativas(result, rules):
    ...
    changes = [s['numero'] for s in sections if s['evaluaciones']['general']['puntaje'] is not None
               and s['evaluaciones']['ultimo_periodo']['puntaje'] is not None
               and s['evaluaciones']['general']['puntaje'] != s['evaluaciones']['ultimo_periodo']['puntaje']]
    values['avances_municipales'] = (
        ('Hay diferencias entre el puntaje general y el del último periodo en los indicadores ' + ', '.join(map(str, changes)) + '. '
         if changes else 'No se detectan diferencias entre los puntajes disponibles de ambos periodos. ')
